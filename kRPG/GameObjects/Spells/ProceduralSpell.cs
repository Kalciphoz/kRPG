    ...
    public class ProceduralSpell
    {
        ...
        public void Draw(SpriteBatch spriteBatch, Vector2 position, float scale)
        {
            try
            {
                ...
                if (full && Remaining == 0 && character.Mana >= ManaCost(character)) spriteBatch.Draw(GFX.GFX.GothicLetter[Key], position, Color.White, scale);
                else if (full || character.SpellCraftingGui.GuiActive) spriteBatch.Draw(GFX.GFX.GothicLetter[Key], position, new Color(143, 143, 151), scale);
            }
            catch (Exception e)
            {
