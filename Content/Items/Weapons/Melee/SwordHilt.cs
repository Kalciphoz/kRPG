    ...
    public class SwordHilt
    {
            ...
            int critBonus = 0, bool spear = false, bool autoSwing = false, int mana = 0, float scale = 0f)
        {
            ...
            if (Main.netMode != NetmodeID.Server)
                Texture = ModLoader.GetMod(Constants.ModName).GetTexture("Content/GFX/Items/Swordhilts/" + texture);
        ...
        {
            if (Main.netMode != NetmodeID.Server)
                SpearTexture = ModLoader.GetMod(Constants.ModName).GetTexture("Content/GFX/Projectiles/SpearMounts/" + texture);
