...
namespace kRPG_mp
{
    ...
    public class kRPG : Mod
    {
        ...
        public override void ModifyInterfaceLayers(List<GameInterfaceLayer> layers)
        {
            ...
            foreach (string s in ModLoader.GetLoadedMods())
            {
                if (s == "MagicStorage")
                {
                    layers.Insert(layers.FindIndex(layer => layer.Name == "Vanilla: Inventory"), new LegacyGameInterfaceLayer("MagicStorage: StorageAccess", new GameInterfaceDrawMethod(InterfaceHelper.DrawStorageGUI), InterfaceScaleType.UI));
                }
            }
        ...
        {
            if (Main.netMode == 2 || (Main.gameMenu && !(Main.playerInventory && Main.autoPause))) return;
            try
            {
